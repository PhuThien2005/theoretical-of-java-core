...
## no12_exception_handling — 2026-06-25 (Updated 2026-07-24)
...
- `theory/01-what-is-an-exception-concepts.md`: Added 2 Why sections: "Why Java Has Checked and Unchecked Exceptions" (external failure vs programming bug philosophy, compiler enforcement rationale, checked vs unchecked method signature code, cause-effect chain) and "Why Catching Broad Exception Types Is Dangerous" (bug masking mechanism, narrow vs broad catch scope mental model, broad catch NPE masking code, cause-effect chain). *Updated on 2026-07-24*: Fully rewrote the file to eliminate all copy-paste boilerplate text. Added explicit definitions for `Error` and `Exception`, mapped out a detailed hierarchical structure, and deeply analyzed common runtime Errors (`OutOfMemoryError`, `StackOverflowError`, `NoClassDefFoundError`) and Exceptions (Checked: `IOException`, `ClassNotFoundException`; Unchecked: `NullPointerException`, `ArrayIndexOutOfBoundsException`, `ArithmeticException`, `ClassCastException`, `NumberFormatException`) with step-by-step mechanisms, detailed mental models (including sequence and hierarchy diagrams), runnable code examples with expected outputs, and cause-effect chains.
