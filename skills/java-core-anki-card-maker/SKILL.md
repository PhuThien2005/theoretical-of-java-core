...
- When a card depends on an external source or exact Java behavior, include a reference URL in `Source`, `Extra`, or `Explanation` so the learner can trace the idea back.
